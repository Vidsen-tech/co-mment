...
// This defines the structure of our form data for submission
...
const WorkCreateModal: React.FC<Props> = ({ open, onClose, newsList }) => {
    ...
    // Use a separate state for credits to handle their unique ID logic
    const [credits, setCredits] = useState<{ hr: CreditItem[], en: CreditItem[] }>({ hr: [], en: [] });

    const { data, setData, post, processing, errors, reset } = useForm<CreateWorkForm>();
    ...
    const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setImagePreviews(prev => {
            ...
            // If no thumbnail is set, make the first one the default
    ...
    const updateImage = (index: number, field: 'author' | 'is_thumbnail', value: string | boolean) => {
        setImagePreviews(prev => prev.map((img, idx) => {
            ...
            // If setting a new thumbnail, unset the old one
    ...
    // ★★★ THE SINGLE, CORRECT `updateShowing` FUNCTION ★★★
    const updateShowing = (id: string, field: 'performance_date' | 'location' | 'news_id' | 'external_link', value: string | number | null) => {
        setFormShowings(prev => prev.map(s => {
            if (s.id === id) {
                ...
                // Logic to ensure only one link type (internal or external) is used
    ...
    const submit = (e: React.FormEvent) => {
        ...
        const formatCreditsForSubmission = (creditItems: CreditItem[]) => {
            return creditItems.reduce((acc, credit) => {
        ...
        const finalData = {
            ...
            showings: formShowings.map(({ id, ...rest }) => rest), // Remove temporary UUIDs
        };
    ...
    // Set form data whenever our local states change
    useEffect(() => {
        setData(currentData => ({
            ...currentData,
            translations: {
                hr: { title: currentData.translations?.hr?.title ?? '', description: currentData.translations?.hr?.description ?? '', credits: {} },
                en: { title: currentData.translations?.en?.title ?? '', description: currentData.translations?.en?.description ?? '', credits: {} },
            },
