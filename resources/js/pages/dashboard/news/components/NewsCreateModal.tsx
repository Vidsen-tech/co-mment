// resources/js/pages/dashboard/news/components/NewsCreateModal.tsx
...
// Represents a single image file for preview purposes in the UI.
interface ImagePreview {
    url: string;
    name: string;
}

// ★★★ FIX: The form's data structure is updated to include all fields that will be submitted.
...
const NewsCreateModal: React.FC<Props> = ({ open, onClose, newsTypes }) => {
    // State to control which language's input fields are visible.
    const [activeLocale, setActiveLocale] = useState<'hr' | 'en'>('hr');
    // ★★★ FIX: Image previews are now managed in a separate state, solely for display.
    const [imagePreviews, setImagePreviews] = useState<ImagePreview[]>([]);


    // ★★★ FIX: Inertia's form helper is now the single source of truth for all form data.
    ...
    // A stable function to handle all cleanup when the dialog closes.
    const handleClose = useCallback(() => {
        // Revoke object URLs to prevent memory leaks
        imagePreviews.forEach(p => URL.revokeObjectURL(p.url));
        setImagePreviews([]);
        reset(); // Resets useForm data and errors
        setActiveLocale('hr');
        onClose();
    }, [imagePreviews, onClose, reset]);

    // Effect to clear form state if the modal is closed externally.
    useEffect(() => {
        if (!open) {
            clearErrors();
        }
    ...
    // ★★★ FIX: All file/image data manipulation now uses `setData` to update the form state.
    const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setData(prevData => {
            ...
            // If there's no thumbnail, make the first image the default thumbnail.
            ...
            return {
                ...
                thumbnail_index: updatedThumbnailIndex
            };
        });
    ...
    // ★★★ FIX: Helper functions now update the form data via `setData`.
    const removeImage = (indexToRemove: number) => {
        // Revoke the URL from the preview state to prevent memory leaks
    ...
    // ★★★ FIX: The submit handler is now much simpler.
    // We just call `post` and Inertia handles the FormData conversion automatically.
    ...
    const getError = (field: string) => errors[field as keyof typeof errors];
                ...
                <ScrollArea className="max-h-[85vh] pr-6">
                    ...
                    <form onSubmit={submit} id="news-create-form" className="space-y-6 py-4">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <ToggleGroup type="single" value={activeLocale} onValueChange={(v: 'hr' | 'en') => v && setActiveLocale(v)} aria-label="Odabir jezika">
                        ...
                        {Object.keys(data.translations).map((locale) => (
                            <div key={locale} className={cn('space-y-4', activeLocale === locale ? 'block' : 'hidden')}>
                                ...
                                <div>
                                    ...
                                    <Textarea id={`excerpt-${locale}`} rows={6} value={data.translations[locale as 'hr'|'en'].excerpt} onChange={e => setData(d => ({...d, translations: {...d.translations, [locale]: {...d.translations[locale as 'hr'|'en'], excerpt: e.target.value}} }))} className={cn(getError(`translations.${locale}.excerpt`) && 'border-red-500')} />
                        ...
                        <div>
                            <Label htmlFor="source">Izvor (Link)</Label>
                            <Input id="source" value={data.source} onChange={e => setData('source', e.target.value)} placeholder="https://primjer.com/izvor" className={cn(errors.source && 'border-red-500')} />
                        ...
                        <div>
                            ...
                            <div className="mt-1 border border-dashed rounded-md p-4 text-center cursor-pointer hover:border-primary transition-colors">
                                <Label htmlFor="image-upload-create" className="cursor-pointer flex flex-col items-center justify-center">
                                    <UploadCloud className="h-8 w-8 text-muted-foreground" />
                                    <span className="mt-2 font-medium text-primary">Kliknite za upload</span>
                                    <span className="text-xs text-muted-foreground">ili povucite slike ovdje</span>
                                </Label>
                                <Input id="image-upload-create" type="file" multiple accept="image/*" onChange={onFileChange} className="sr-only" />
                            </div>
                            {errors.images && <p className="text-xs text-red-500 mt-1">{String(errors.images)}</p>}
                            {errors.thumbnail_index && <p className="text-xs text-red-500 mt-1">{errors.thumbnail_index}</p>}
                        </div>
                        {/* --- Image Previews (Now using the separate preview state) --- */}
